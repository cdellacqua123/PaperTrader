...
export default async function getFooterStocks(ticker) {
    ...
    const url = `https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=${ticker}&apikey=DY7ON0PJPEXR89XR`;
    ...
    let arg = { 'ticker': ticker, 'last': 0, 'change': 0};
    let response = await pull;
    let table = await response.data['Time Series (Daily)'];
    let keys = Object.keys(table)
    // console.log(keys[0])
    // console.log(keys[1])
    arg['last'] = parseFloat(table[keys[0]]['4. close'])
    let change = (arg['last'] - parseFloat(table[keys[1]]['4. close'])).toFixed(2)
    arg['change'] = change
